"""Batch episode collection script for television_lab teleoperation."""
...
from tv_isaaclab import launch_simulation_app, IsaacLabEnvBridge, EpisodeRecorder, add_app_launcher_args
...
def _resize_pair(left_img, right_img, height, width):
    import cv2
    left_resized = cv2.resize(left_img, (width, height), interpolation=cv2.INTER_LINEAR)
    right_resized = cv2.resize(right_img, (width, height), interpolation=cv2.INTER_LINEAR)
...
def collect_episodes(
    ...
    ngrok: bool = False,
    simulation_app: bool = None,
):
    """Collect multiple episodes of teleoperation data."""
    
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Initialize environment and teleoperator
    teleoperator = VuerTeleop(
        (Path(__file__).parent / retarget_config).as_posix(),
        ngrok=ngrok
    )
    mapper = ActionMapper((Path(__file__).parent / action_mapping).as_posix())
    ...
    print(f"    Task: {task}")
    print(f"    Action dim: {mapper.action_dim}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--output_dir", type=str, default="../data/recordings/isaaclab", help="Output directory for episodes")
    ...
    # For non-simulation environments, we can skip AppLauncher
    simulation_app = None
    try:
        simulation_app = launch_simulation_app(args)
    except Exception as e:
        print(f"[!] Could not launch simulation app: {e}")
